...
function setupConnection(clusterKey, applicationKey, secure) {
	...
	connection = connectionManager.create({
		...
		authenticationEndPoint: new EventSockets.AuthenticationEndPoint('/endpoints/authentication.aspx')
	});
...
function setupButtons() { 
	...
	$("#subscribe").click(function () {
		...
		var channel = connection.create("private", "channel");
	...
	$("#rest").click(function () {
		$.ajax({ url: 'rest.aspx?channelPrefix=private&nocache=' + Math.floor(Math.random()*99999999) });
	});
	...
	$("#trigger").click(function () {
		var channel = connection.channels.getValue("private.channel");
